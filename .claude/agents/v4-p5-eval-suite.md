# v4-p5-eval-suite: Run a small eval (HumanEval-100 subset) and compare pass-rate

**Phase:** 5 — Validation against reference outputs
**Complexity:** M
**Branch:** `feat/v4-p5-eval-suite`
**Dependencies:** `v4-p5-logit-diff-test`

## Scope

Verifies semantic correctness on top of the per-token logit check. **Out:** anything beyond a single eval.
...
This spec was scaffolded by the v4-roadmap builder phase from row `v4-p5-eval-suite` of `docs/plans/v4-roadmap.md`. Re-read the roadmap before you start — the surrounding context (Metal gap analysis in §2, V4 delta in §3, risks in §5) is load-bearing.
