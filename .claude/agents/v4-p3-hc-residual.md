# v4-p3-hc-residual: Manifold-Constrained Hyper-Connections forward path

**Phase:** 3 — V4 forward pass (CPU + Metal)
**Complexity:** M
**Branch:** `feat/v4-p3-hc-residual`
**Dependencies:** `v4-p2-model-loader-stub`

## Scope

Implement `dsv4_hc_pre`/`dsv4_hc_post` analogs as plain `mul_mat + scale + add` chains. No new ggml ops. Add `hyper_connection.sinkhorn_iters` loop. Tested on CPU first; Metal needs no new kernels.
...
This spec was scaffolded by the v4-roadmap builder phase from row `v4-p3-hc-residual` of `docs/plans/v4-roadmap.md`. Re-read the roadmap before you start — the surrounding context (Metal gap analysis in §2, V4 delta in §3, risks in §5) is load-bearing.
