# v4-p5-tool-call-fixture: End-to-end tool-call regression test

**Phase:** 5 — Validation against reference outputs
**Complexity:** S
**Branch:** `feat/v4-p5-tool-call-fixture`
**Dependencies:** `v4-p5-logit-diff-test`, `v4-p4-tool-calling`

## Scope

Once `v4-p4-tool-calling` lands, capture a reference tool-call completion from the official V4 API and add a fixture in `tests/test-tool-call.cpp`. Separated from `v4-p5-logit-diff-test` so the latter can land before tool-calling is stable.
...
This spec was scaffolded by the v4-roadmap builder phase from row `v4-p5-tool-call-fixture` of `docs/plans/v4-roadmap.md`. Re-read the roadmap before you start — the surrounding context (Metal gap analysis in §2, V4 delta in §3, risks in §5) is load-bearing.
