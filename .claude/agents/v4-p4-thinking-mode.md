# v4-p4-thinking-mode: Reasoning / `<think>` mode plumbing

**Phase:** 4 — Chat template / tool-calling
**Complexity:** S
**Branch:** `feat/v4-p4-thinking-mode`
**Dependencies:** `v4-p4-chat-template`

## Scope

Surface `thinking_mode` as a CLI flag for `llama-cli` and `llama-server`. Map to template variant in `LLM_CHAT_TEMPLATE_DEEPSEEK_4`. **Out:** changing the sampling pipeline.
...
This spec was scaffolded by the v4-roadmap builder phase from row `v4-p4-thinking-mode` of `docs/plans/v4-roadmap.md`. Re-read the roadmap before you start — the surrounding context (Metal gap analysis in §2, V4 delta in §3, risks in §5) is load-bearing.
